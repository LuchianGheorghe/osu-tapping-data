...
import pandas as pd
import requests
from tapping_data.helpers import get_downloaded_maps_path, get_parsed_maps_path, round_divisor, create_empty_series
...
def get_beatmap_data(map_id):
	maps_path = os.path.join(get_downloaded_maps_path(), str(map_id) + '.osu')
	if not os.path.exists(maps_path):
		url = f'https://osu.ppy.sh/osu/{map_id}'
		response = requests.get(url)
		if response.text != '':
			with open(maps_path, 'wb') as map:
				map.write(response.content)
		else:
			raise ValueError(f'Invalid beatmap ID: {map_id}')
	return BeatmapIO.open_beatmap(maps_path)
